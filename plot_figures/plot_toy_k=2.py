...
noise_points_x1 = np.random.rand(15) * (0.5) + 0.5  # 在[-0.5, 0.5]范围内生成50个随机横坐标
noise_points_y1 = np.random.rand(15) * (-1) # 在[-1, -0.5]范围内生成50个随机纵坐标
...
noise_points2 = np.random.rand(10, 2) * np.array([0.5, 0.5]) + np.array([2.0, 6])  # 在指定范围内生成随机点
X = np.vstack([X, noise_points2])

# 使用K均值聚类np.arange(start, stop, step)
